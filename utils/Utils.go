...
// strRequest: API路由路径
// return: 请求结果
func ApiKeyGet(cfg *config.RestConfig, mapParams map[string]string, strRequestPath string) string {
	if !cfg.Logon {
...
// strRequest: API路由路径
// return: 请求结果
func ApiKeyPost(cfg *config.RestConfig, mapParams map[string]string, strRequestPath string) string {
	if !cfg.Logon {
		log.Fatal("Please config HuobiAPI AccessKey and SecretKey")
	}
